<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="font-bold text-2xl text-gray-800 leading-tight">Edit Fasilitas</h2>
                <p class="text-sm text-gray-500 mt-1">Perbarui informasi sarana: {{ $facility->name }}</p>
            </div>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-xl shadow-gray-100 sm:rounded-3xl overflow-hidden border border-gray-100">
                <form action="{{ route('admin.facility.update', $facility->id) }}" method="POST" enctype="multipart/form-data" class="p-8 md:p-12 space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
                        <div class="lg:col-span-2 space-y-6">
                            <div>
                                ...
                                <x-text-input id="name" name="name" type="text" class="block w-full border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 rounded-2xl py-3" :value="old('name', $facility->name)" required />
                            ...
                            <div>
                                ...
                                <textarea id="description" name="description" class="editor">{{ old('description', $facility->description) }}</textarea>
                        ...
                        <div class="space-y-6">
                            <div class="bg-gray-50/50 p-6 rounded-3xl border border-gray-100 space-y-6">
                                ...
                                <div class="relative group border-2 border-dashed border-gray-200 rounded-2xl p-4 hover:border-indigo-400 transition-colors bg-white">
                                    <div id="preview-container" class="{{ $facility->image ? '' : 'hidden' }} mb-4">
                                        <img id="preview-image" src="{{ $facility->image ? asset('storage/' . $facility->image) : '#' }}" class="rounded-xl w-full aspect-video object-cover shadow-sm">
                                    </div>
                                    
                                    <div id="upload-placeholder" class="{{ $facility->image ? 'hidden' : '' }} py-8 text-center space-y-2">
                                        <svg class="mx-auto h-12 w-12 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                        <p class="text-xs font-bold text-gray-500">Ganti foto fasilitas</p>
                                    </div>
                                ...
                                <div class="p-4 bg-amber-50 rounded-2xl border border-amber-100">
                                    <p class="text-[10px] text-amber-700 leading-relaxed font-medium">
                                        <span class="font-black uppercase tracking-tighter">Info:</span> Biarkan kosong jika tidak ingin mengganti foto yang sudah ada.
                                    </p>
                                </div>
